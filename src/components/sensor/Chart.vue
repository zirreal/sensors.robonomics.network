...
function showMultipleSeries(ids = []) {
  ...
  const GROUPS = {
    dust: ['pm10', 'pm25', 'pm2_5'],
    noise: ['noise', 'noiseavg', 'noisemax']
  };

  // Show only one legend per group if any visible
  Object.values(GROUPS).forEach(groupIds => {
    const visibleSeries = groupIds
      .map(id => chart.get(id))
      .filter(s => s && s.visible);

    if (visibleSeries.length) {
      const legendOwner = visibleSeries.find(s => idsSet.has(s.options.id)) || visibleSeries[0];

      groupIds.forEach(id => {
        const s = chart.get(id);
        if (!s) return;
        const shouldShowLegend = s === legendOwner;
        if (s.options.showInLegend !== shouldShowLegend) {
          s.update({ showInLegend: shouldShowLegend }, false);
        }
      });
    }
  });
...
async function restoreSingleSeries(id) {
  ...
  store.setCurrentChartMeasure(activeType.value);
...
watch(
  ...
  async (newLen, oldLen) => {
    ...
    store.setCurrentChartMeasure(ns.id);
